/* eslint-disable @typescript-eslint/no-explicit-any, no-console, react-hooks/set-state-in-effect */
'use client';
...
export default function SkipController({
  ...
  // 🔑 全屏模式下跳过配置组件的显示控制
  const [showConfigPanelInFullscreen, setShowConfigPanelInFullscreen] =
    useState(false);
  ...
  useEffect(() => {
    const checkFullscreen = () => {
      ...
      if (fullscreenElement) {
        // 查找 artplayer 容器
        const artplayerContainer = fullscreenElement.querySelector(
          '.artplayer',
        ) as HTMLElement;
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(
        'skipControllerFullscreenPosition',
        JSON.stringify(fullscreenPosition),
      );
    }
  }, [fullscreenPosition]);
  ...
  const renderConfigPanel = () => {
    ...
    if (!isFullscreen && isCollapsed) {
      ...
      const floatingBall = (
        ...
        >
          {/* 圆形悬浯球（不靠近边缘时） */}
          {!isSnapped && (
            <div className='bg-linear-to-br from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-full shadow-2xl border-2 border-white/30 transition-all w-14 h-14 flex flex-col items-center justify-center'>
              <svg
          ...
          {isSnapped && (
            <div
              className={`bg-linear-to-br from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white shadow-2xl border border-white/30 transition-all
            ...
            >
              <div
                className={`flex items-center gap-2 ${snap.edge === 'top' || snap.edge === 'bottom' ? 'flex-row' : 'flex-col'}`}
              >
                {openingSegment && (
                  <div className='text-center'>
                    <div className='text-[10px] opacity-75'>🎬片头</div>
                    <div className='text-xs font-bold'>
                      {formatTime(openingSegment.end)}
                    </div>
                  </div>
                )}
                {endingSegment && (
                  <div className='text-center'>
                    ...
                    <div className='text-xs font-bold'>
                      {formatTime(endingSegment.start)}
                    </div>
                  </div>
                )}
              </div>
    ...
    const configPanel = (
      <div
        ...
        onTouchStart={
          isFullscreen ? handleFullscreenTouchStart : handleTouchStart
        }
        onMouseEnter={() => {
          setIsHovering(true);
          // 鼠标进入时清除自动收起定时器
        ...
        <div className='p-3'>
          ...
          <div className='drag-handle flex items-center justify-between mb-2 cursor-move select-none'>
            <h4 className='font-medium text-gray-900 dark:text-gray-100 text-sm flex items-center'>
              ...
              <span
                className={`ml-2 text-xs ${isFullscreen ? 'text-gray-300' : 'text-gray-500 dark:text-gray-400'}`}
              >
                可拖动
              </span>
            </h4>
          ...
          <div className='space-y-1'>
            {actualSegments.map((segment, index) => (
              ...
              >
                <span
                  className={`${isFullscreen ? 'text-white' : 'text-gray-800 dark:text-gray-200'} flex-1 mr-2`}
                >
                  ...
                  {segment.autoSkip && (
                    <span
                      className={`ml-1 px-1 ${isFullscreen ? 'bg-green-500/80 text-white' : 'bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400'} rounded text-xs`}
                    >
                      自动
                    </span>
                  )}
            ...
            <div className='space-y-4 bg-linear-to-br from-purple-50/50 to-pink-50/50 dark:from-purple-900/20 dark:to-pink-900/20 p-4 rounded-xl border border-purple-100/50 dark:border-purple-800/50 backdrop-blur-sm'>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    batchSettings.endingMode === 'remaining' ? '2:00' : '20:00'
                  }
                />
                <button
