...
export default function CollapseButton({
  ...
  return (
    ...
    >
      ...
      <svg
        className={`relative z-10 w-4 h-4 text-gray-500 dark:text-gray-400 transition-transform duration-200 ${isCollapsed ? 'rotate-180' : 'rotate-0'
          }`}
        fill='none'
        stroke='currentColor'
        viewBox='0 0 24 24'
      ...
      <div className='absolute -top-0.5 -right-0.5 z-20'>
        <div className='relative'>
          <div
            className={`absolute inset-0 rounded-full blur-sm opacity-75 ${isCollapsed ? 'bg-orange-400 animate-pulse' : 'bg-green-400'
              }`}
          ></div>
          <div
            className={`relative w-2 h-2 rounded-full shadow-lg ${isCollapsed
