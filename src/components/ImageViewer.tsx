...
const ImageViewer: React.FC<ImageViewerProps> = ({
  ...
  useEffect(() => {
    let showAnimationId: number;
    let animateInId: number;
    let animateOutId: number;
    let timer: ReturnType<typeof setTimeout>;

    if (isOpen) {
      showAnimationId = requestAnimationFrame(() => {
        setIsVisible(true);
        animateInId = requestAnimationFrame(() => {
          setIsAnimating(true);
        });
      });
    } else {
      animateOutId = requestAnimationFrame(() => {
    ...
    return () => {
      if (showAnimationId) cancelAnimationFrame(showAnimationId);
      if (animateInId) cancelAnimationFrame(animateInId);
      if (animateOutId) cancelAnimationFrame(animateOutId);
    ...
    <div className='fixed inset-0 z-[9999] flex items-center justify-center p-0 sm:p-4'>
      ...
      >
        <Image
          ...
          width={1200}
          height={1800}
          className='h-auto max-h-[100vh] max-w-[100vw] w-auto object-contain sm:max-h-[90vh] sm:max-w-[90vw]'
