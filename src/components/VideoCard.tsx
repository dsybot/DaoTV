...
const VideoCard = forwardRef<VideoCardHandle, VideoCardProps>(
  function VideoCard(
    ...
    return (
      <>
        <div
          className='@container group relative w-full rounded-lg bg-transparent cursor-pointer transition-all duration-300 ease-in-out hover:scale-[1.05] hover:z-500'
        ...
        >
          ...
          >
            ...
            <Image
              ...
              className={`${origin === 'live' ? 'object-contain' : 'object-cover'} transition-opacity duration-300 ease-out ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
            ...
            {config.showSourceName &&
              ...
              (() => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`relative inline-flex items-center justify-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full border border-gray-300/60 dark:border-gray-600/60 text-gray-600 dark:text-gray-400 transition-all duration-300 ease-out group-hover:shadow-md group-hover:scale-105 ${colorClasses}`}
                      style={
                        {
                          WebkitUserSelect: 'none',
                          userSelect: 'none',
                    ...
                    >
                      ...
                      {/* 左侧装饰点 - 手机端隐藏 */}
                      <span
                        className={`relative w-1.5 h-1.5 rounded-full bg-gray-400 dark:bg-gray-500 transition-all duration-300 hidden sm:inline-block shrink-0 ${dotColor}`}
                      ></span>

                      {origin === 'live' && (
                        <Radio
                          size={12}
                          className={`relative inline-block transition-all duration-300 shrink-0 ${iconColor}`}
                        />
                      )}

                      <span className='relative font-semibold whitespace-nowrap'>
                        {displayText}
                      </span>
