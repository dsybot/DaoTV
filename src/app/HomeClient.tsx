...
import type { HomePageConfig } from '@/hooks/useHomePageQueries';
...
type HomePageDisplayConfig = Required<HomePageConfig> & {
...
type HomeAction =
  ...
  | { type: 'SET_HOME_PAGE_CONFIG'; payload: HomePageDisplayConfig };
...
const DEFAULT_HOME_PAGE_CONFIG: HomePageDisplayConfig = {
  showHeroBanner: true,
...
interface HomeClientProps {
  initialConfig?: Partial<HomePageDisplayConfig>;
}

function HomeClient({ initialConfig }: HomeClientProps) {
  const normalizedInitialConfig = useMemo<HomePageDisplayConfig>(
    () => ({
  ...
  const [state, dispatch] = useReducer(homeReducer, {
    ...
    homePageConfig: normalizedInitialConfig,
  });
  ...
  const {
    ...
    showHotMovies: normalizedInitialConfig.showHotMovies,
    showHotTvShows: normalizedInitialConfig.showHotTvShows,
    showHotVariety: normalizedInitialConfig.showHotVariety,
    showNewAnime: normalizedInitialConfig.showNewAnime,
    showHotShortDramas: normalizedInitialConfig.showHotShortDramas,
  });
  ...
  useEffect(() => {
    ...
    if (!state.homePageConfig.showUpcomingReleases) return;
    ...
    <PageLayout>
      ...
      <div className='dao-home-shell overflow-visible pb-32 md:pb-safe-bottom'>
        ...
        {state.homePageConfig.showHeroBanner &&
          !loading &&
          ((state.homePageConfig.showHotMovies && hotMovies.length > 0) ||
            (state.homePageConfig.showHotTvShows && hotTvShows.length > 0) ||
            (state.homePageConfig.showHotVariety &&
              hotVarietyShows.length > 0) ||
            (state.homePageConfig.showNewAnime && hotAnime.length > 0)) && (
