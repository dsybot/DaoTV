...
async function fetchFromShortDramaSource(api: string, size: number) {
  ...
  const categories = (listData.class || []) as ShortDramaCategoryItem[];

  // 查找所有短剧相关分类（父分类 + 子分类标签）
  const shortDramaCategories = categories.filter(
    (cat) =>
      cat.type_name &&
      SHORT_DRAMA_KEYWORDS.some((keyword) =>
  ...
  const subCategory =
    shortDramaCategories.find((cat) => !PARENT_ONLY.includes(cat.type_name)) ??
  ...
  const items = (data.list || []) as ShortDramaSourceItem[];
...
async function getRecommendedShortDramasInternal(category?: number, size = 10) {
  try {
    ...
    // 合并所有成功的结果
    const allItems: ShortDramaRecommendItem[] = [];
