/* eslint-disable @typescript-eslint/no-explicit-any, react-hooks/exhaustive-deps, no-console, react-hooks/unsupported-syntax, unused-imports/no-unused-vars */
...
function PlayPageClient() {
  ...
  const playerCleanupCallbacksRef = useRef<Array<() => void>>([]);

  const registerPlayerCleanup = (cleanup: () => void) => {
    playerCleanupCallbacksRef.current.push(cleanup);
  };

  const runPlayerCleanupCallbacks = () => {
    const callbacks = playerCleanupCallbacksRef.current;
    playerCleanupCallbacksRef.current = [];

    for (const cleanup of callbacks.reverse()) {
      try {
        cleanup();
      } catch (err) {
        console.warn('[Cleanup] 播放器清理回调出错:', err);
      }
    }
  };
  ...
  const isAudioTrackSwitchingRef = useRef(isAudioTrackSwitching);
  ...
  useEffect(() => {
    ...
    isAudioTrackSwitchingRef.current = isAudioTrackSwitching;
  }, [
    blockAdEnabled,
    customAdFilterCode,
    needPrefer,
  ...
  useEffect(() => {
    const fetchTmdbData = async () => {
      ...
      if (movieDetails?.first_aired) {
        ...
        const dateMatch =
          movieDetails.first_aired.match(/^(\d{4}-\d{2}-\d{2})/);
        if (dateMatch) {
          airDate = dateMatch[1];
        }
      ...
      try {
        console.log(
          `[TMDB] 开始获取演员数据: ${videoTitle}, type: ${type}, season: ${detectedSeason}, airDate: ${airDate}`,
        );
        ...
        if (response.ok) {
          ...
          if (
            ...
            console.log(
              `[TMDB] 使用匹配到的第 ${data.matched_season || detectedSeason} 季的分集信息`,
            );
  ...
  const lightweightPreference = async (
    ...
    console.log(
      '轻量级优选结果:',
      sortedResults.map(
        (r) => `${r.source.source_name}: ${r.pingTime}ms (权重: ${r.weight})`,
      ),
    );
  ...
  useEffect(() => {
    ...
    const processAudioTracks = (rawTracks: any[]) => {
      ...
      if (selectedTrackIndex < 0) {
        selectedTrackIndex =
          mappedTracks.find((track) => track.isDefault)?.index ??
          mappedTracks[0].index;
      }
  ...
  const buildAudioTrackControl = () => {
    const currentTrack = audioTracks.find((track) =>
    ...
    const selector = audioTracks.map((track) => {
      ...
      return {
        html: `${selected ? '▶ ' : ''}${escapeAudioTrackHtml(track.name || '音轨')}`,
    ...
    return {
      ...
      onSelect: function (item: any) {
        const selectedTrack = audioTracksRef.current.find((track) => {
          if (track.index !== item.trackIndex) return false;
          if (typeof item.trackHlsIndex === 'number') {
            return track.hlsIndex === item.trackHlsIndex;
          }
          return true;
        });
        if (selectedTrack) {
  ...
  // Portal 容器将在 ArtPlayer 初始化后设置为 $player 元素
  ...
  useEffect(() => {
    const fetchSourceDetail = async (
      ...
      try {
        ...
        console.error(
          `获取视频详情失败 (尝试 ${retryCount + 1}/${maxRetries + 1}):`,
          err,
        );
        // 如果还有重试次数，等待后重试
  ...
  const handleEpisodeChange = async (episodeNumber: number) => {
    if (episodeNumber >= 1 && episodeNumber <= totalEpisodes) {
      ...
      try {
        ...
        if (
          record &&
          record.index - 1 === targetIndex &&
          record.play_time > 0
        ) {
          resumeTimeRef.current = record.play_time;
          console.log(
            `🎯 切换到第${episodeNumber}集，恢复历史进度: ${record.play_time.toFixed(2)}s`,
          );
        } else {
          resumeTimeRef.current = 0;
          console.log(`🔄 切换到第${episodeNumber}集，从头播放`);
        }
      } catch (err) {
      ...
      try {
        const newUrl = new URL(window.location.href);
        newUrl.searchParams.set('index', targetIndex.toString());
        window.history.replaceState({}, '', newUrl.toString());
      } catch (err) {
        ...
        if (needsUpdate) {
          ...
          saveFavoriteMutation.mutate({
            ...
            favorite: {
              ...
              origin: favoriteToUpdate.origin,
              type: contentType,
  ...
  useEffect(() => {
    ...
    const initPlayer = async () => {
      ...
      try {
              ...
              hls.on(
                ...
                (_event: any, data: any) => {
                  ...
                  if (preferredLang) {
                    const preferredTrack = mappedTracks.find(
                      (track) =>
                        normalizeAudioLang(track.language) === preferredLang,
                    );
                    if (
                      preferredTrack &&
        ...
        const revealControlsAfterFullscreenChange = () => {
          const revealControls = () => {
            const art = artPlayerRef.current;
            if (!art?.controls) return;

            art.controls.show = true;
            const bottom = art.template?.$bottom as HTMLElement | undefined;
            if (bottom) {
          ...
          [100, 220, 420].forEach((delay) => {
            window.setTimeout(revealControls, delay);
          });
        };
        ...
        const handleDocumentFullscreenChange = () => {
          ...
          if (!art || !fullscreenElement) return;
        ...
        artPlayerRef.current.on('ready', async () => {
          ...
          if (timeParam && artPlayerRef.current) {
            ...
            if (!isNaN(seekTime) && seekTime > 0) {
              ...
              const watchRoomSeekTimer = setTimeout(() => {
                if (artPlayerRef.current) {
                  artPlayerRef.current.currentTime = seekTime;
                }
              }, 500); // 延迟确保播放器完全就绪
              registerPlayerCleanup(() => clearTimeout(watchRoomSeekTimer));
            }
          }
          ...
          if ((isIOS || isSafari) && artPlayerRef.current.muted) {
            ...
            let firstPlayVolumeTimer: NodeJS.Timeout | null = null;
            const handleFirstPlay = () => {
              firstPlayVolumeTimer = setTimeout(() => {
            ...
            registerPlayerCleanup(() => {
              artPlayerRef.current?.off('video:play', handleFirstPlay);
              if (firstPlayVolumeTimer) clearTimeout(firstPlayVolumeTimer);
            });
          }
          ...
          const fixDanmakuProgressConflict = () => {
            ...
            let immediateRestoreTimer: NodeJS.Timeout | null = null;
            let handleProgressMouseDown: ((event: MouseEvent) => void) | null =
              null;
            let handleDocumentMouseMove: (() => void) | null = null;
            let handleDocumentMouseUp: (() => void) | null = null;

            const danmakuConflictTimer = setTimeout(() => {
              ...
              handleProgressMouseDown = (event: MouseEvent) => {
                // 只有左键才开始拖拽检测
                if (event.button === 0) {
              ...
              // 立即执行一次恢复
              immediateRestoreTimer = setTimeout(immediateRestore, 100);
            }, 1500); // 等待弹幕插件加载

            registerPlayerCleanup(() => {
              clearTimeout(danmakuConflictTimer);
              if (immediateRestoreTimer) clearTimeout(immediateRestoreTimer);
              if (danmakuResetInterval) clearInterval(danmakuResetInterval);
              if (progressControl && handleProgressMouseDown) {
                progressControl.removeEventListener(
                  'mousedown',
                  handleProgressMouseDown,
                );
              }
          ...
          const addMobileDanmakuToggle = () => {
            ...
            let configButtonEl: Element | null = null;
            let configClickHandler: ((e: Event) => void) | null = null;
            let toggleButtonEl: HTMLElement | null = null;
            let toggleClickHandler: ((e: MouseEvent) => void) | null = null;

            const mobileDanmakuToggleTimer = setTimeout(() => {
            ...
            registerPlayerCleanup(() => {
              clearTimeout(mobileDanmakuToggleTimer);
              if (configButtonEl && configClickHandler) {
                configButtonEl.removeEventListener('click', configClickHandler);
              }
              if (toggleButtonEl && toggleClickHandler) {
                toggleButtonEl.removeEventListener(
                  'click',
                  toggleClickHandler,
                  true,
                );
                delete toggleButtonEl.dataset.customDanmuToggleBound;
              }
            });
          };
          ...
          const externalDanmuLoadTimer = setTimeout(async () => {
            try {
              const result = await loadExternalDanmu(); // 这里会检查开关状态，返回 { count, data }
          ...
          registerPlayerCleanup(() => clearTimeout(externalDanmuLoadTimer));
        ...
        artPlayerRef.current.on('video:canplay', () => {
          ...
          // 音轨切换完成
          if (isAudioTrackSwitchingRef.current) {
            setIsAudioTrackSwitching(false);
          }
          ...
          if ((isIOS || isSafari) && artPlayerRef.current.paused) {
            ...
            let autoPlayStartTimer: NodeJS.Timeout | null = null;
            let firstInteractionVolumeTimer: NodeJS.Timeout | null = null;
            let handleFirstUserInteraction: (() => Promise<void>) | null = null;

            const delayAutoPlayRetry = (delay: number) =>
            ...
            const tryAutoPlay = async () => {
              try {
                ...
                const attemptPlay = async (): Promise<boolean> => {
                  if (autoPlayCancelled || !artPlayerRef.current) return false;
                  playAttempts++;
                ...
                if (!success) {
                  ...
                  if (artPlayerRef.current) {
                    ...
                    handleFirstUserInteraction = async () => {
                      ...
                      try {
                        await artPlayerRef.current.play();
                        // 首次成功播放后恢复正常音量
                        firstInteractionVolumeTimer = setTimeout(() => {
                          if (
                            artPlayerRef.current &&
            ...
            // 延迟尝试，避免与进度恢复冲突
            autoPlayStartTimer = setTimeout(tryAutoPlay, 200);
            registerPlayerCleanup(() => {
              autoPlayCancelled = true;
              if (autoPlayStartTimer) clearTimeout(autoPlayStartTimer);
              if (firstInteractionVolumeTimer) {
                clearTimeout(firstInteractionVolumeTimer);
              }
              if (handleFirstUserInteraction) {
                artPlayerRef.current?.off(
                  'video:play',
                  handleFirstUserInteraction,
                );
          ...
          const restorePlaybackStateTimer = setTimeout(() => {
            if (
          ...
          registerPlayerCleanup(() => clearTimeout(restorePlaybackStateTimer));
  ...
  useEffect(() => {
    ...
    const updateEpisodeSelectorButtonVisibility = () => {
      ...
      const shouldShow =
        isFullscreen || isWebFullscreen || isEpisodeSelectorCollapsed;
      ...
      <PageLayout activePath='/play'>
            ...
            >
              ...
              >
                <EpisodeSelector
                  ...
                  tmdbEpisodes={
                    searchType === 'tv' && !isVarietyShow ? tmdbEpisodes : []
                  }
        ...
        {showEpisodePopup &&
          ...
          createPortal(
            ...
            >
              <div
                className='relative w-full h-full max-w-4xl mx-auto p-8 md:p-12'
              ...
              >
                ...
                <div
                  className='relative w-full h-full overflow-hidden px-12'
                  style={{ pointerEvents: 'auto' }}
                >
                  <EpisodeSelector
                    totalEpisodes={totalEpisodes}
