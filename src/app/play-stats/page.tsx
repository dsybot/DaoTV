                    ...
                    >
                            ...
                            <div className='min-w-0 flex-1'>
                              ...
                              {userStat.lastLoginIp && (
                                <p className='text-xs text-gray-500 dark:text-gray-400 flex items-center flex-wrap gap-1'>
                                  <span>登入IP:</span>
                                  <span className='font-mono break-words'>{userStat.lastLoginIp}</span>
                              ...
                              {userStat.lastLoginDevice && (
                                <p className='text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1'>
                                  <span>
                                    {userStat.lastLoginDevice === 'mobile' ? '📱'
                                      : userStat.lastLoginDevice === 'tablet' ? '📟'
                                        : '💻'}
                                  </span>
                                  <span className='truncate min-w-0'>
                                    {userStat.lastLoginOs || parseUserAgent(userStat.lastLoginDevice)}
                                    {userStat.lastLoginBrowser ? ` · ${userStat.lastLoginBrowser}` : ''}
                                  </span>
                                </p>
                              )}
                              <div className='text-xs text-gray-500 dark:text-gray-400'>
                      ...
                      {expandedUsers.has(userStat.username) && (
                        <div className='p-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700'>
                          {/* 登入信息卡片 */}
                          {(userStat.lastLoginIp || userStat.lastLoginDevice) && (
                                ...
                                {userStat.lastLoginDevice && (
                                  <div className='flex items-center gap-1.5 text-xs text-gray-700 dark:text-gray-300'>
                                    ...
                                    <span className='truncate'>
                                      {[userStat.lastLoginOs, userStat.lastLoginBrowser].filter(Boolean).join(' · ')}
                                    </span>
                                  </div>
                                )}
                              </div>
