...
const isIOS = typeof window !== 'undefined' && /iPad|iPhone|iPod/i.test(userAgent) && !('MSStream' in window);
...
function getDoubanImageProxyConfig(): {
  ...
  if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
    ...
    // 自动修复：如果 localStorage 或 RUNTIME_CONFIG 是 'direct'，自动改为 'server'
    let effectiveStoredType = storedType;
    if (storedType === 'direct') {
      effectiveStoredType = 'server';
      // 自动更新 localStorage，避免下次还是 'direct'
...
function getBangumiImageProxyConfig(): {
  ...
  return {
    proxyType: bangumiImageProxyType,
    proxyUrl: bangumiImageProxyUrl,
  };
}
...
export function processImageUrl(originalUrl: string): string {
  ...
  // Bangumi 图片代理（lain.bgm.tv 在部分网络环境无法直接访问）
  if (originalUrl.includes('lain.bgm.tv') || originalUrl.includes('bgm.tv/pic')) {
    const { proxyType, proxyUrl } = getBangumiImageProxyConfig();
    switch (proxyType) {
      case 'cmliussss':
...
interface VideoPlayRuntimeConfig {
  VIDEO_PROXY_ENABLED?: boolean;
  VIDEO_PROXY_URL?: string;
}

type VideoPlayWindow = Window & {
  RUNTIME_CONFIG?: VideoPlayRuntimeConfig;
};
...
function getVideoPlayProxyConfig(): { enabled: boolean; proxyUrl: string } {
  ...
  const rc = (window as VideoPlayWindow).RUNTIME_CONFIG;
  return {
