...
export class SqliteStorage implements IStorage {
  ...
  async deleteUser(userName: string): Promise<void> {
    ...
    try {
      ...
      this.db
        ...
        .run(userName);
      this.db
        .prepare('DELETE FROM login_logs WHERE username = ?')
        .run(userName);
      this.db.exec('COMMIT');
    } catch (e) {
      this.db.exec('ROLLBACK');
  ...
  async addLoginLog(loginLog: LoginLog): Promise<void> {
    this.db
      .prepare(
        'INSERT OR REPLACE INTO login_logs (id, username, login_time, value) VALUES (?, ?, ?, ?)',
      )
      .run(
        loginLog.id,
        loginLog.username,
  ...
  async getLoginLogs(limit = 100): Promise<LoginLog[]> {
    const rows = this.db
      ...
      .all(limit) as Array<{ value: string }>;
    return rows.map((row) => JSON.parse(row.value) as LoginLog);
  }
  ...
  async getLastLoginLog(username: string): Promise<LoginLog | null> {
    const row = this.db
      .prepare(
        'SELECT value FROM login_logs WHERE username = ? ORDER BY login_time DESC LIMIT 1',
      )
      .get(username) as { value: string } | undefined;
    return row ? (JSON.parse(row.value) as LoginLog) : null;
  }
}
